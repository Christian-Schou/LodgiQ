...
public static class ClaimsPrincipalExtensions
{
    public static Guid GetUserId(this ClaimsPrincipal? principal)
    {
        ...
        return Guid.TryParse(userId, out Guid parsedUserId)
            ? parsedUserId
            : throw new LodginglyException("User identifier is not available in claims");
    }

    public static string GetIdentityId(this ClaimsPrincipal? principal)
    {
        return principal?.FindFirst(ClaimTypes.NameIdentifier)?.Value ??
               throw new LodginglyException("User identity is not available in claims");
    }

    public static HashSet<string> GetPermissions(this ClaimsPrincipal? principal)
    {
        IEnumerable<Claim> permissionClaims = principal?.FindAll(CustomClaims.Permission) ??
                                              throw new LodginglyException("Permissions are not available in claims");
